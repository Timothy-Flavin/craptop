...
class BatchedGridEnv(gym.vector.VectorEnv):
    def __init__(
        ...
        share_danger: If True, when a radio communication fires between agents i and j,
            ...
            window (7×7 tiles around j). Has no effect when communication_prob <= 0.
        """
        ...
        # global_discovered offset (absolute)
        global_disc_offset = (
            agent_locs_offset + n_agents * 2 + n_agents * fms + n_agents * 2 * n_agents
        )
        self.sl_global_disc = slice(global_disc_offset, global_disc_offset + fms)
    ...
    def get_gravity_attractions(
        self, feature_type, agent_mask=None, pow=2, normalize=False, local=False
    ):
        """
        ...
        Args:
            ...
            local: If True, only consider tiles/agents within the agent's VIEW_RANGE (default False)
        ...
        gravity_array = self.env.get_gravity_attractions(
            agent_mask, feature_type, pow, normalize, local
        )
